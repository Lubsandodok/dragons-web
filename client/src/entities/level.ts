...
export class Level implements Physical {
  ...
  private setUpPhysicsForBorder(border: Container): Rapier.Collider {
    ...
    const borderColliderDesc = Rapier.ColliderDesc.cuboid(
      border.width,
      border.height
    );
  ...
  setUpLevelPhysicsForContainers() {
    ...
    const leftGroundColliderDesc = Rapier.ColliderDesc.cuboid(
      GROUND_WIDTH,
      WORLD_SIDE_Y
    ...
    const rightGroundColliderDesc = Rapier.ColliderDesc.cuboid(
      GROUND_WIDTH,
      WORLD_SIDE_Y
  ...
  private makeHorizontalBorder(side: BorderSide): Container {
    ...
    // TODO
    for (
      let x = 2 * borderResource.height;
      x < WORLD_SIDE_X;
      x += borderResource.height - 1
    ) {
  ...
  private makeVerticalBorder(side: BorderSide): Container {
    const container = new Container();
    const borderResource =
      side === BorderSide.LEFT
        ? resources.groundBorderDark
        : resources.groundBorderLight;
